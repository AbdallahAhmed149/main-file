...
# dic1 = {"study_hours_per_day": "mean", "netflix_hours": ["median", "std"]}
# print(df.agg(dic1))

# print(df.groupby("gender").agg(dic1))
# print(df.groupby("gender").agg(
#     study_hours_mean = ("study_hours_per_day", "mean"),
#     netflix_median = ("netflix_hours", "median"),
#     netflix_std = ("netflix_hours", "std")
# ))

# ( removing outliers ) --> Important
# sns.boxplot(data=df, x="gender", y="social_media_hours")
# plt.show()

# group = df.groupby("gender")["social_media_hours"].agg(
#     Q1 = lambda x : np.quantile(x, 0.25),
#     Q3 = lambda x : np.quantile(x, 0.75),
#     IQR = lambda x : iqr(x),
#     Lower_Point = lambda x : np.quantile(x, 0.25) - 1.5 * iqr(x),
#     Higher_Point = lambda x : np.quantile(x, 0.75) + 1.5 * iqr(x),
# )
# group = df.groupby("gender")["social_media_hours"].agg("median")

# print(df[(df["gender"] == "Male") & (df["social_media_hours"] > 5.7)].index)
# print(group[group.index == "Male"]["Higher_Point"])

# idx = df[(df["gender"] == "Male") & ((df["social_media_hours"] > 5.70) | (df["social_media_hours"] < -0.70))].index
...
# sns.boxplot(data=male, x="gender", y="social_media_hours")
# plt.show()

# q1_male = male["social_media_hours"].quantile(0.25)
# q3_male = male["social_media_hours"].quantile(0.75)
# iqr = q3_male - q1_male
# low = q1_male - 1.5 * iqr
# high = q3_male + 1.5 * iqr
...
# male.drop(male[(male["social_media_hours"] < low) | (male["social_media_hours"] > high)].index, inplace=True)
# sns.boxplot(data=male, x="gender", y="social_media_hours")
# plt.show()
...
# objt = df2.select_dtypes("object")
# for i in objt:
#     df2[i] = df2[i].fillna(df2[i].mode()[0])
# print(df2.isna().sum())

# # ( fill None values that Gender = Male with the mean of the gender male likewise with female )
# dictionary = df2.groupby("Gender")["Salary"].mean().to_dict()
# df2["Salary"] = df2["Salary"].fillna(df2["Gender"].map(dictionary))

# print(df2[df2["Gender"].str.contains("Male")])
# print(df2[df2["Team"].str.contains("Marketing|Finance")])
# print(df2[df2["Team"].str.contains("^M")])
...
# ]
# conditions = [
#     df2["Team"].str.contains("Marketing"),
#     df2["Team"].str.contains("Client Services"),
#     df2["Team"].str.contains("Finance"),
#     df2["Team"].str.contains("Legal"),
#     df2["Team"].str.contains("Product"),
#     df2["Team"].str.contains("Engineering"),
#     df2["Team"].str.contains("Business Development"),
#     df2["Team"].str.contains("Human Resources"),
#     df2["Team"].str.contains("Sales"),
#     df2["Team"].str.contains("Distribution"),
# ]
# df2["Team_Category"] = np.select(condlist=conditions, choicelist=team_categories, default="Other")
# print(df2[["Team", "Team_Category"]].head())

# sns.catplot(df2, x="Team_Category", kind="count", hue="Team_Category")
# plt.show()

# # ( Replacing multiple phrases in string )
# df2["Team"] = df2["Team"].str.replace("Marketing", "Marketer")
# df2["Team"] = df2["Team"].str.replace("Client Services", "Call Center")
# df2["Team"] = df2["Team"].str.replace("Finance", "Financialist")
# df2["Team"] = df2["Team"].str.replace("Legal", "Lawyer")
# df2["Team"] = df2["Team"].str.replace("Product", "Production")
# df2["Team"] = df2["Team"].str.replace("Engineering", "Engineer")
# df2["Team"] = df2["Team"].str.replace("Business Development", "Business DEV")
# df2["Team"] = df2["Team"].str.replace("Human Resources", "HR")
# df2["Team"] = df2["Team"].str.replace("Sales", "Salling")
# df2["Team"] = df2["Team"].str.replace("Distribution", "Distribut")
# print(df2["Team"].head())

# # ( make a column called "std_dev" and fill it with the std of the gender )
# df2["std_dev"] = df2.groupby("Gender")["Salary"].transform(lambda x: x.std())
# print(df2[["Gender", "std_dev"]].head())


# ( to convert to datetime )
# the is 1st way, and this is best
df3 = pd.read_csv("divorce.csv", parse_dates=["divorce_date", "dob_man", "dob_woman", "marriage_date"])

# # this is the 2nd way
# df3["divorce_date"] = pd.to_datetime(df3["divorce_date"])
# df3["dob_man"] = pd.to_datetime(df3["dob_man"])
# df3["dob_woman"] = pd.to_datetime(df3["dob_woman"])
# df3["marriage_date"] = pd.to_datetime(df3["marriage_date"])

df3["marriage_day"] = df3["marriage_date"].dt.day
df3["marriage_month"] = df3["marriage_date"].dt.month
df3["marriage_year"] = df3["marriage_date"].dt.year

# sns.lineplot(data=df3, x="marriage_year", y="marriage_duration", ci=None)
# sns.barplot(data=df3, x="marriage_year", y="marriage_duration", ci=None)
# plt.show()

# print(df3.corr(numeric_only=True))
# sns.heatmap(df3.corr(numeric_only=True), annot=True)
# sns.lmplot(data=df3, x="income_man", y="income_woman")
# plt.show()

# print(df3["marriage_date"].min())
# print(df3["marriage_date"].max())

# sns.pairplot(data=df3)
# sns.pairplot(data=df3, vars=["income_man", "income_woman", "marriage_duration"])
# plt.show()

# sns.histplot(data=df3, x="marriage_duration", hue="education_man")
# sns.histplot(data=df3, x="marriage_duration", hue="education_man", cumulative=True)
# sns.kdeplot(data=df3, x="marriage_duration", hue="education_man", warn_singular=False, cut=0, cumulative=True)
# plt.show()

# df3["man_age_marriage"] = df3["marriage_year"] - df3["dob_man"].dt.year
# df3["woman_age_marriage"] = df3["marriage_year"] - df3["dob_woman"].dt.year

# sns.scatterplot(data=df3, x="woman_age_marriage", y="man_age_marriage")
# plt.show()

# # These tow lines are identical give the same resutl but the first is more organized
# print(pd.crosstab(index=df["gender"], columns=df["parental_education_level"]))
# print(df[["gender", "parental_education_level"]].value_counts())

# cross = pd.crosstab(index=df["gender"], columns=df["parental_education_level"])
# print(cross["Bachelor"])

# These lines are identical but the first two are identical exactly and you can use any one of them
# print(pd.crosstab(index=df["gender"], columns=df["parental_education_level"], values=df["age"], aggfunc="mean"))
...
# sns.heatmap(data.corr(numeric_only=True), annot=True)
# sns.lmplot(data=data, x="salary_usd", y="years_experience")
# plt.show()

# objects = data.select_dtypes("object")
# print(objects.nunique())
